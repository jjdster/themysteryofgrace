...
import { newBodyStudyData } from '../data/newBodyStudyData';
...
const AIGuide = ({ 
  ...
  const [messages, setMessages] = useState<{ role: 'user' | 'guide'; text: string }[]>([
    { role: 'guide', text: `Hello! I'm your study guide for this lesson on "${lesson.title}". What questions do you have about the differences between our earthly body and our future heavenly body?` }
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const prompt = `
        ...
        Your goal is to help the user understand the biblical distinction between the Earthly Body (corruptible, mortal, vile) and the Heavenly Body (incorruptible, immortal, glorious) based ONLY on the provided scriptures and source material, always applying the principle of 'rightly dividing the Word of Truth' (2 Timothy 2:15).
...
const Quiz = ({ 
  ...
  const getEncouragingRemark = (scorePercent: number) => {
    if (scorePercent === 100) return "Excellent work! You have a clear vision of your heavenly hope.";
    if (scorePercent >= 80) return "Great job! You understand the glorious transformation awaiting us.";
    if (scorePercent >= 60) return "Good effort! Review the nature of the resurrection body to strengthen your hope.";
    return "Don't be discouraged! These are deep spiritual truths. Review the scriptures and try again.";
  };
...
export default function NewBodyStudy() {
  const { user } = useAuth();
  ...
  const visibleStudyData = newBodyStudyData.map(module => ({
    ...module,
      ...
      <div className="min-h-screen bg-secondary-light flex items-center justify-center p-4">
        ...
        >
          ...
          <p className="text-xl text-primary/60 mb-12 leading-relaxed">
            You have successfully completed the study on <span className="text-accent font-bold">The New Body</span>. 
            May this glorious hope strengthen your walk as you look for the Savior from heaven.
          </p>
          ...
          <Link to="/prophecy-mystery-study" className="block bg-secondary-light/50 p-8 rounded-3xl mb-12 text-left hover:bg-secondary-light transition-colors group">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-primary/60">Distinguish between God's earthly and heavenly programs.</p>
              </div>
            ...
            <div className="p-6 bg-primary rounded-3xl text-secondary shadow-lg relative overflow-hidden">
              <div className="relative z-10">
                ...
                <p className="text-xs text-secondary/70 leading-relaxed font-light">
                  Our citizenship is in heaven. We are just passing through this earthly tent.
                </p>
              </div>
