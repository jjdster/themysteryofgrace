...
export default function SevenOnesStudy() {
  const { user } = useAuth();
  ...
  const currentModule = sevenOnesData[currentModuleIdx];
  ...
  const nextLesson = () => {
    if (currentLessonIdx < currentModule.lessons.length - 1) {
      ...
      setShowQuiz(false);
    } else if (currentModuleIdx < sevenOnesData.length - 1) {
    ...
    return (
      <div className="min-h-screen bg-secondary-light flex items-center justify-center p-4">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-2xl w-full bg-white rounded-[3rem] p-12 text-center shadow-2xl">
          ...
          <p className="text-xl text-primary/60 mb-12">You have completed "The Seven Ones of Ephesians".</p>
    ...
    <div className="min-h-screen bg-secondary-light">
      <div className="bg-white border-b border-primary/10 py-8 px-4">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-serif font-bold text-primary">The Seven Ones of Ephesians</h1>
            ...
            <div className="space-y-2">
              {sevenOnesData.map((mod, idx) => (
                <div key={mod.id} className={`p-4 rounded-xl border transition-all ${idx === currentModuleIdx ? 'bg-white border-accent shadow-sm' : 'opacity-40'}`}>
                  <span className="text-xs font-bold text-accent block mb-1">Module {idx}</span>
